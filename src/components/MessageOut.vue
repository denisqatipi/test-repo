...
const handleTemplateUpload = async (event: Event) => {
  ...
  if (target.files && target.files[0]) {
    try {
      ...
      const response = await api.uploadJsonFile(file)
...
<template>
  <div class="space-y-8">
    ...
    <div class="space-y-2">
      ...
      <div class="flex gap-3">
        <div class="flex-1 relative">
          ...
          <input
            type="text"
            readonly
            :value="templateFile?.name || 'No file selected'"
      ...
      <p class="text-xs text-gray-500">Upload a template file to define the output structure</p>
    </div>
  </div>
</template>
