...
const buildTreeFromJson = (jsonData: any, parentId: string = ''): TreeNode[] => {
  if (!jsonData) return []
  ...
  if (Array.isArray(jsonData)) {
    return jsonData.map((item, index) => {
      ...
      return {
        ...
        expanded: false,
        children: typeof item === 'object' ? buildTreeFromJson(item, currentId) : undefined
      }
    })
  }
  ...
  return Object.entries(jsonData).map(([key, value], index) => {
    ...
    return {
      ...
      expanded: false,
      children: typeof value === 'object' ? buildTreeFromJson(value, currentId) : undefined
    }
  })
}
...
watch(() => props.data, (newData) => {
  if (newData) {
    treeData.value = buildTreeFromJson(newData)
  }
}, { immediate: true })
